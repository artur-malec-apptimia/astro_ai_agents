...
export async function refreshGoogleToken(
  ...
  if (!data.access_token) throw new Error(`Google token error: ${data.error ?? 'no access_token'}`);
...
export async function searchZendeskTickets(
  ...
  const res = await fetch(
    ...
    { headers: { Authorization: `Basic ${buildZendeskAuth(agentEmail, apiKey)}` } },
  );
  if (!res.ok) throw new Error(`Zendesk API error: ${res.status}`);
  const data = (await res.json()) as {
    results: { id: number; subject: string; status: string; priority: string | null; created_at: string }[];
  };
