...
const getCalendarEventsTool = createTool({
  ...
  execute: async ({ date }: { date?: string }) => {
    ...
    const events = await getCalendarEvents(accessToken, env("GOOGLE_CALENDAR_ID"), date);
...
const getZendeskTicketsTool = createTool({
  ...
  inputSchema: z.object({
    query: z.string().describe("Customer name, email, or domain to search open tickets for"),
  }),
...
const getHubSpotDealsTool = createTool({
  ...
  inputSchema: z.object({
    query: z.string().describe("Company or deal name to search active deals for"),
  }),
  execute: async ({ query }: { query: string }) => {
