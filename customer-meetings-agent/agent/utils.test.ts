...
describe('refreshGoogleToken', () => {
  test('returns access_token on success', async () => {
    ...
    spies.push(spy);
    const token = await refreshGoogleToken('client-id', 'client-secret', 'refresh-tok');
  ...
  test('throws on non-ok response', async () => {
    ...
    spies.push(spy);
    await expect(
      refreshGoogleToken('id', 'secret', 'refresh'),
    ).rejects.toThrow('400');
  });
...
describe('getCalendarEvents', () => {
  test('GETs the calendar events endpoint with time range', async () => {
    ...
    expect(url).toContain('2026-05-22T00:00:00Z');
    expect(url).toContain('2026-05-22T23:59:59Z');
  });
...
describe('searchZendeskTickets', () => {
  test('GETs the Zendesk search endpoint', async () => {
    ...
    spies.push(spy);
    await searchZendeskTickets('https://company.zendesk.com', 'agent@example.com', 'key', 'acme');
    const [url] = spy.mock.calls[0] as [string];
...
describe('searchHubSpotDeals', () => {
  ...
  test('handles missing deal properties', async () => {
    ...
    expect(deals[0]).toEqual({ id: 'd-2', name: '', amount: null, stage: null, closeDate: null });
  });
