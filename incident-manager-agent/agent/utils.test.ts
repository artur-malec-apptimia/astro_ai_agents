...
describe('createIncidentInNotion', () => {
  ...
  test('sends correct properties and children blocks in body', async () => {
    ...
    expect(body.properties.Name).toEqual({ title: [{ text: { content: 'DB Outage' } }] });
    expect(body.properties.Status).toEqual({ select: { name: 'In progress' } });
    expect(body.properties['Severity Level']).toEqual({ select: { name: 'Critical' } });
    expect(body.properties['Incident Date']).toEqual({ date: { start: '2026-05-20' } });
    expect(body.properties['Slack Channel ID']).toEqual({ rich_text: [{ text: { content: 'C123' } }] });
...
describe('updateIncidentInNotion', () => {
  ...
  test('throws on non-ok response from PATCH properties', async () => {
    ...
    await expect(
      updateIncidentInNotion('bad', 'page-1', {
        status: 'Done', severity_level: 'High',
        detail_summary: 'x', engineering_update: 'x', support_update: 'x',
      }),
    ).rejects.toThrow('401');
  });
  ...
  test('archives existing blocks and appends new summary blocks', async () => {
    const spy = spyOn(global, 'fetch')
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: 'page-1' }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ results: [{ id: 'block-a' }, { id: 'block-b' }] }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({}), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({}), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({}), { status: 200 }));
    spies.push(spy);
    ...
    const [archiveAUrl, archiveAInit] = spy.mock.calls[2] as [string, RequestInit];
    expect(archiveAUrl).toBe('https://api.notion.com/v1/blocks/block-a');
    expect(JSON.parse((archiveAInit as RequestInit).body as string)).toEqual({ archived: true });
