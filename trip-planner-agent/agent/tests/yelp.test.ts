...
describe('searchYelp', () => {
  ...
  test('sends bearer token from YELP_API_KEY', async () => {
    await searchYelp('museums', 'New York', 5);
    const opts = (global.fetch as unknown as ReturnType<typeof mock>).mock.calls[0][1] as RequestInit;
    expect((opts.headers as Record<string, string>)['Authorization']).toBe('Bearer test-yelp-key');
  });
  ...
  test('throws if YELP_API_KEY is not set', async () => {
    delete process.env.YELP_API_KEY;
    await expect(searchYelp('museums', 'New York', 5)).rejects.toThrow('YELP_API_KEY not set');
  });
  ...
  test('throws on non-ok response', async () => {
    ...
    await expect(searchYelp('museums', 'New York', 5)).rejects.toThrow('Yelp API error: 429');
  });
});
