...
describe('createNotionTripPlanTemplate', () => {
  ...
  test('makes exactly 3 Notion API calls', async () => {
    global.fetch = makeFetchSequence([{ id: 'trip-page-id' }, { id: 'db-id' }, {}]);
    await createNotionTripPlanTemplate('Paris Trip', ['Passport', 'Sunscreen']);
    expect((global.fetch as unknown as ReturnType<typeof mock>).mock.calls).toHaveLength(3);
  });

  test('returns trip_page_id and trip_schedule_database_id', async () => {
    global.fetch = makeFetchSequence([{ id: 'trip-page-id-abc' }, { id: 'db-id-xyz' }, {}]);
    const result = await createNotionTripPlanTemplate('Paris Trip', ['Passport']);
    expect(result).toEqual({ trip_page_id: 'trip-page-id-abc', trip_schedule_database_id: 'db-id-xyz' });
  });
  ...
  test('first call creates page under NOTION_PARENT_PAGE_ID', async () => {
    ...
    const firstCall = (global.fetch as unknown as ReturnType<typeof mock>).mock.calls[0];
  ...
  test('second call creates database with correct schema', async () => {
    ...
    await createNotionTripPlanTemplate('Paris Trip', []);
    const secondCall = (global.fetch as unknown as ReturnType<typeof mock>).mock.calls[1];
  ...
  test('third call appends packing list blocks', async () => {
    ...
    const thirdCall = (global.fetch as unknown as ReturnType<typeof mock>).mock.calls[2];
    ...
    const itemTexts = body.children
      ...
      .map((b: { bulleted_list_item: { rich_text: Array<{ text: { content: string } }> } }) => b.bulleted_list_item.rich_text[0].text.content);
    expect(itemTexts).toEqual(['Passport', 'Camera']);
  });

  test('throws if NOTION_BEARER_TOKEN is not set', async () => {
    delete process.env.NOTION_BEARER_TOKEN;
    await expect(createNotionTripPlanTemplate('Paris Trip', [])).rejects.toThrow('NOTION_BEARER_TOKEN not set');
  });

  test('throws if NOTION_PARENT_PAGE_ID is not set', async () => {
    delete process.env.NOTION_PARENT_PAGE_ID;
    await expect(createNotionTripPlanTemplate('Paris Trip', [])).rejects.toThrow('NOTION_PARENT_PAGE_ID not set');
  });
});
...
describe('addWeatherToNotionDatabase', () => {
  ...
  test('posts to /v1/pages', async () => {
    await addWeatherToNotionDatabase('db-id', 'Monday', '2025-06-02', 'Sunny 72°F', 'Visit Eiffel Tower', 'Dinner at Café de Flore');
  ...
  test('sends all day properties in body', async () => {
    await addWeatherToNotionDatabase('db-id-xyz', 'Tuesday', '2025-06-03', 'Cloudy 65°F', 'Louvre Museum', 'Bistro lunch');
    const body = JSON.parse(
      ((global.fetch as unknown as ReturnType<typeof mock>).mock.calls[0][1] as RequestInit).body as string,
    );
    expect(body.parent.database_id).toBe('db-id-xyz');
    expect(body.properties['Day of Week'].title[0].text.content).toBe('Tuesday');
    expect(body.properties['Date'].date.start).toBe('2025-06-03');
    expect(body.properties['Weather'].rich_text[0].text.content).toBe('Cloudy 65°F');
    expect(body.properties['Activities Planned'].rich_text[0].text.content).toBe('Louvre Museum');
    expect(body.properties['Dining'].rich_text[0].text.content).toBe('Bistro lunch');
  });

  test('returns the created page id', async () => {
    const result = await addWeatherToNotionDatabase('db-id', 'Monday', '2025-06-02', 'Sunny', 'Beach', 'Seafood');
    expect((result as { id: string }).id).toBe('entry-id');
  });

  test('throws if NOTION_BEARER_TOKEN is not set', async () => {
    delete process.env.NOTION_BEARER_TOKEN;
    await expect(addWeatherToNotionDatabase('db-id', 'Monday', '2025-06-02', 'Sunny', 'Beach', 'Seafood')).rejects.toThrow('NOTION_BEARER_TOKEN not set');
  });
});
