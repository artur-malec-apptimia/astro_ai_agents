...
describe('getWeatherForecast', () => {
  ...
  test('returns parsed JSON response', async () => {
    const result = await getWeatherForecast(40.7128, -74.006, '2025-06-01', '2025-06-02');
  ...
  test('throws on non-ok response', async () => {
    ...
    await expect(getWeatherForecast(0, 0, '2025-06-01', '2025-06-02')).rejects.toThrow(
      'Weather forecast API error: 500',
    );
  });
});
...
describe('getHistoricalWeather', () => {
  ...
  test('throws on non-ok response', async () => {
    ...
    await expect(getHistoricalWeather(0, 0, '2024-06-01', '2024-06-02')).rejects.toThrow(
