...
export async function createNotionTripPlanTemplate(
  ...
  if (!pageRes.ok) {
    throw new Error(`Notion create page error: ${pageRes.status} ${await pageRes.text()}`);
  }
  ...
  if (!dbRes.ok) {
    throw new Error(`Notion create database error: ${dbRes.status} ${await dbRes.text()}`);
  }
  ...
  const packingBlocks = [
    {
      object: 'block',
      type: 'heading_2',
      heading_2: { rich_text: [{ type: 'text', text: { content: '🎒 Packing List' } }] },
    },
  ...
  if (!packRes.ok) {
    throw new Error(`Notion add packing list error: ${packRes.status} ${await packRes.text()}`);
  }
  ...
  const res = await fetch(`${NOTION_API}/pages`, {
    ...
    body: JSON.stringify({
      ...
      properties: {
        ...
        'Weather': { rich_text: [{ type: 'text', text: { content: weather_summary } }] },
        'Activities Planned': { rich_text: [{ type: 'text', text: { content: activities_planned } }] },
        'Dining': { rich_text: [{ type: 'text', text: { content: dining_plan } }] },
      },
    }),
  });
  if (!res.ok) {
    throw new Error(`Notion add entry error: ${res.status} ${await res.text()}`);
  }
  return res.json();
}
